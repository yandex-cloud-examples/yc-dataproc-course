    ...
    dynamic "subcluster_spec" {
      ...
      content {
        ...
        dynamic "autoscaling_config" {
          for_each = subcluster_spec.value.autoscaling_config.enabled ? [1] : []
          content {
            max_hosts_count         = subcluster_spec.value.autoscaling_config.max_hosts_count
            measurement_duration    = subcluster_spec.value.autoscaling_config.measurement_duration
            warmup_duration         = subcluster_spec.value.autoscaling_config.warmup_duration
            stabilization_duration  = subcluster_spec.value.autoscaling_config.stabilization_duration
            preemptible             = subcluster_spec.value.autoscaling_config.preemptible
            decommission_timeout    = subcluster_spec.value.autoscaling_config.decommission_timeout
            cpu_utilization_target  = subcluster_spec.value.autoscaling_config.cpu_utilization_target
              }
            }
          }
        }
